		...
		if(brandName && brandStatus) {
			...
			$.ajax({
				...
				success:function(response) {
  	  			...
  	  			$('#add-brand-messages').html('<div class="alert alert-success" style="border-left: 5px solid green;">'+
            '<button type="button" class="close" data-dismiss="alert">&times;</button>'+
            '<strong><i class="fas fa-ok-sign"></i></strong> '+ response.messages +
          '</div>');
...
function editBrands(brandId = null) {
	if(brandId) {
		...
		$.ajax({
			url: 'includes/fetchSelectedBrand.php',
			type: 'post',
					...
					if(brandName && brandStatus) {
						...
						$.ajax({
							...
							success:function(response) {
			  	  			...
			  	  			$('#edit-brand-messages').html('<div class="alert alert-success" style="border-left: 5px solid green;">'+
			            '<button type="button" class="close" data-dismiss="alert">&times;</button>'+
			            '<strong><i class="fas fa-ok-sign"></i></strong> '+ response.messages +
			          '</div>');
					...
					$.ajax({
						...
						success:function(response) {
							...
							if(response.success == true) {
								...
								$('.remove-messages').html('<div class="alert alert-success" style="border-left: 5px solid green;">'+
			            '<button type="button" class="close" data-dismiss="alert">&times;</button>'+
			            '<strong><i class="fas fa-ok-sign"></i></strong> '+ response.messages +
			          '</div>');
